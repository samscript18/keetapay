...
export function LocaleSwitcher({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    const markActive = () => setLuminaActive(true);
    window.addEventListener("keetapay:lumina-translation", markActive);
    return () => window.removeEventListener("keetapay:lumina-translation", markActive);
  }, []);
  ...
  return (
    <label className="relative inline-flex items-center gap-2 rounded-[8px] border border-white/10 bg-white/[0.04] px-2 text-sm text-white/70">
      ...
      {luminaActive && <span className="h-2 w-2 rounded-full bg-accent shadow-[0_0_10px_rgba(39,241,154,0.9)]" title={t("luminaActive")} aria-label={t("luminaActive")} />}
