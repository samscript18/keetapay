...
export function SendCard({
  ...
  const [recipient, setRecipient] = useState<SelectedUser | undefined>(
    lockedRecipient,
  );
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      paymentStatus.setStatus(
        "processing",
        "Publishing your transfer to Keeta testnet.",
      );
      ...
      if (mode === "batch") {
        ...
        const result = await api.sendMany(
          authToken,
          { payments },
          controller.signal,
        );
      ...
      const result = await api.send(
        authToken,
        { recipient: `@${recipient.username}`, amount, message },
        controller.signal,
      );
      return {
        label: "KTA sent",
    ...
    <Card>
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/48">
            Send to a username or pay many people at once.
          </p>
        </div>
        {success ? (
          <CheckCircle2 className="text-accent" />
        ) : (
          <Send className="text-white/54" />
        )}
      </div>
        ...
        <div className="mb-4 flex items-center justify-between rounded-[8px] border border-white/10 bg-white/[0.04] p-1">
          {[
            ...
            <button
              ...
              className={
                mode === key
                  ? "w-full rounded-[6px] bg-accent px-3 py-2 text-sm font-bold text-black"
                  : "w-full rounded-[6px] px-3 py-2 text-sm font-semibold text-white/56"
              }
            >
              {label}
            </button>
      ...
      <form onSubmit={submit} className="space-y-3">
        {mode === "batch" ? (
          ...
          <>
            <UsernameSelector
              ...
              locked={Boolean(lockedRecipient)}
            />
            <Input
              value={amount}
        ...
        <Button
          ...
          disabled={mode === "single" && !canSendSingle}
        >
          {mutation.isPending
            ? "Sending..."
            : mode === "batch"
      ...
      {success && (
        <motion.div
          ...
          className="mt-4 rounded-[8px] bg-accent/12 p-3 text-sm text-accent"
        >
    ...
    const id = setTimeout(async () => {
      try {
        const [matches, exact] = await Promise.all([
          ...
          normalized.length >= 3
            ? api.publicUser(normalized).catch(() => null)
  ...
  return (
    <div className="space-y-2">
      ...
      {checking ? (
        ...
        <p className="text-sm text-coral">That username does not exist.</p>
      ) : null}
...
function BatchPayments({
  ...
  onChange: (rows: BatchPaymentRow[]) => void;
}) {
  const validCount = useMemo(
    () => rows.filter((row) => row.recipient?.username && row.amount).length,
    [rows],
  );
        ...
        >
          ...
          <UsernameSelector
            ...
            onChange={(recipient) =>
              onChange(
                rows.map((item) =>
                  item.id === row.id ? { ...item, recipient } : item,
                ),
              )
            }
          />
