...
export default function TransactionsPage() {
  ...
  const history = historyQuery.data?.needsUsername
    ? []
    : (historyQuery.data?.history ?? []);
  const username = historyQuery.data?.needsUsername
    ? undefined
    : historyQuery.data?.user.username;
        ...
        <Card>
          <div className="mb-5">
            ...
            <p className="mt-2 text-sm text-white/48">
              Every KTA payment sent or received by your KeetaPay username.
            </p>
          </div>
          {loading ? (
            <TransactionSkeleton />
          ) : (
            <ActivityList transactions={history} currentUsername={username} />
          )}
        </Card>
      </div>
    </AppShell>
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 5 }).map((_, index) => (
        <div
          key={index}
          className="flex items-center gap-3 rounded-[8px] border border-white/10 bg-white/[0.04] p-3"
        >
