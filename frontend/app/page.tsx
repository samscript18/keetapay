...
export default function LandingPage() {
	...
	}, [ready, authenticated, getAccessToken, router, t]);

	const steps = Array.from({ length: 8 }, (_, index) => t(`steps.${index + 1}`));
		...
		<main className="relative overflow-hidden">
			...
			<section className="mx-auto grid min-h-[calc(100vh-64px)] max-w-7xl items-center gap-12 px-4 py-14 lg:grid-cols-[1.05fr_.95fr]">
				<motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
					...
					<h1 className="font-display max-w-4xl text-5xl font-black leading-[1.02] tracking-tight md:text-7xl">
						{t("headline")}
						<span className="block bg-gradient-to-r from-accent via-sky to-coral bg-clip-text text-transparent">{t("headlineAccent")}</span>
					</h1>
					<p className="mt-6 max-w-2xl text-lg leading-8 text-white/62">{t("subtitle")}</p>
					...
					<div className="mt-10 grid gap-4 sm:grid-cols-3">
						<div className="rounded-[10px] border border-white/10 bg-white/[0.05] px-4 py-3">
							<p className="text-xs uppercase tracking-[0.2em] text-white/45">{t("metrics.sendTime")}</p>
							<p className="font-display mt-2 text-2xl font-black">2.4s</p>
						</div>
						<div className="rounded-[10px] border border-white/10 bg-white/[0.05] px-4 py-3">
							<p className="text-xs uppercase tracking-[0.2em] text-white/45">{t("metrics.profiles")}</p>
							<p className="font-display mt-2 text-2xl font-black">1.3k</p>
						</div>
						<div className="rounded-[10px] border border-white/10 bg-white/[0.05] px-4 py-3">
							<p className="text-xs uppercase tracking-[0.2em] text-white/45">{t("metrics.volume")}</p>
				...
				<motion.div id="demo" initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.15 }} className="relative">
					<Card className="shadow-glow animate-pulse">
						<div className="mb-5 flex items-center justify-between">
							<div>
								<p className="text-sm text-white/45">{t("demo.balance")}</p>
						...
						<div className="rounded-[8px] bg-black/30 p-4">
							<p className="text-sm text-white/48">{t("demo.to")}</p>
							<p className="mt-1 text-2xl font-bold">@jayden</p>
							<p className="mt-5 text-sm text-white/48">{t("demo.amount")}</p>
							<p className="mt-1 text-4xl font-black text-accent">5 KTA</p>
							<div className="mt-5 rounded-[8px] bg-white/8 p-3 text-sm text-white/70">{t("demo.note")}</div>
						</div>
						<Button className="mt-5 w-full">{t("demo.action")}</Button>
					</Card>
				</motion.div>
			</section>
			<section className="mx-auto max-w-7xl px-4 py-8">
				<div className="grid gap-4 border border-white/10 bg-white/[0.04] p-5 text-sm text-white/55 md:grid-cols-3">
					<div className="flex items-center gap-3">
						...
						<span>{t("proof.transfers")}</span>
					</div>
					<div className="flex items-center gap-3">
						<Globe2 className="text-accent" />
						<span>{t("proof.identities")}</span>
					</div>
					<div className="flex items-center gap-3">
						<ShieldCheck className="text-accent" />
						<span>{t("proof.checks")}</span>
					</div>
				</div>
			</section>
				...
				<div className="grid gap-4 lg:grid-cols-3 lg:grid-rows-2">
					<Card className="lg:row-span-2">
						<div className="flex items-center gap-3 text-accent">
							<MessageCircle />
							<p className="text-sm uppercase tracking-[0.2em]">{t("features.transferEyebrow")}</p>
						</div>
						<h3 className="font-display mt-4 text-2xl font-black">{t("features.transferTitle")}</h3>
						<p className="mt-4 text-sm leading-6 text-white/54">{t("features.transferDescription")}</p>
						<div className="mt-6 rounded-[10px] border border-white/10 bg-white/[0.06] p-4 text-sm text-white/70">
							<p className="text-xs uppercase tracking-[0.2em] text-white/45">{t("features.nowPlaying")}</p>
					...
					<Card>
						...
						<h3 className="font-display text-xl font-bold">{t("features.identityTitle")}</h3>
						<p className="mt-3 text-sm leading-6 text-white/54">{t("features.identityDescription")}</p>
					</Card>
					<Card>
						<ShieldCheck className="mb-5 text-accent" />
						<h3 className="font-display text-xl font-bold">{t("features.accessTitle")}</h3>
						<p className="mt-3 text-sm leading-6 text-white/54">{t("features.accessDescription")}</p>
					</Card>
					<Card>
						<BadgeCheck className="mb-5 text-accent" />
						<h3 className="font-display text-xl font-bold">{t("features.checksTitle")}</h3>
						<p className="mt-3 text-sm leading-6 text-white/54">{t("features.checksDescription")}</p>
					</Card>
					<Card>
						<Sparkles className="mb-5 text-accent" />
						<h3 className="font-display text-xl font-bold">{t("features.trustTitle")}</h3>
						<p className="mt-3 text-sm leading-6 text-white/54">{t("features.trustDescription")}</p>
					</Card>
				</div>
			</section>
			<section className="mx-auto grid max-w-7xl gap-8 px-4 py-14 lg:grid-cols-2">
				<div>
					<h2 className="font-display text-3xl font-black">{t("how.title")}</h2>
					<p className="mt-3 text-white/55">{t("how.description")}</p>
					<div className="group relative z-50 mt-11 w-full h-[488px] overflow-hidden rounded-[14px] border border-white/10 bg-gradient-to-br from-white/[0.1] via-white/[0.055] to-accent/[0.08] p-4 text-sm text-white/70 shadow-glow backdrop-blur-xl">
						...
						<div className="flex items-center justify-between">
							<p className="text-xs uppercase tracking-[0.2em] text-white/45">{t("how.liveFeed")}</p>
			...
			<section className="mx-auto max-w-7xl px-4 pb-20">
				<div className="grid gap-4 rounded-[16px] border border-white/10 bg-gradient-to-r from-white/[0.08] via-white/[0.03] to-transparent p-8 md:grid-cols-[1.2fr_0.8fr]">
					<div>
						<p className="text-xs uppercase tracking-[0.2em] text-white/45">{t("cta.eyebrow")}</p>
						<h2 className="font-display mt-4 text-3xl font-black md:text-4xl">{t("cta.title")}</h2>
						<p className="mt-4 text-white/55">{t("cta.description")}</p>
					</div>
			...
			<footer className="border-t border-white/10 px-4 py-10 text-center text-sm text-white/42">{t("footer")}</footer>
		</main>
	);
}
