...
export class WalletsService {
	...
	async getBalance(encryptedSeed: string) {
		...
		const account = KeetaNet.lib.Account.fromSeed(seed, 0);
		const client = KeetaNet.UserClient.fromNetwork(this.keetaNetwork(), account as any);
		try {
	...
	async sendKta(encryptedSeed: string, toWalletAddress: string, amount: string) {
		if (!toWalletAddress) throw new BadRequestException("Recipient wallet is missing");
		...
		const client = KeetaNet.UserClient.fromNetwork(this.keetaNetwork(), sender as any);
	...
	decryptSeed(encryptedSeed: string) {
		...
		const plaintext = Buffer.concat([decipher.update(Buffer.from(ciphertextRaw, "base64")), decipher.final()]);
		return plaintext.toString("utf8");
	}

	accountFromEncryptedSeed(encryptedSeed: string) {
		return KeetaNet.lib.Account.fromSeed(this.decryptSeed(encryptedSeed), 0);
	}

	private encryptionKey() {
		const raw = this.config.get<string>("MASTER_ENCRYPTION_KEY");
		if (!raw) throw new InternalServerErrorException("MASTER_ENCRYPTION_KEY is missing");
		const key = Buffer.from(raw, "base64");
		if (key.length !== 32) throw new InternalServerErrorException("MASTER_ENCRYPTION_KEY must be 32 bytes base64");
		return key;
	}

	private keetaNetwork() {
		const network = this.config.get<string>("KEETA_NETWORK") ?? "test";
		if (["main", "test"].includes(network)) return network as KeetaNetwork;
		throw new InternalServerErrorException("KEETA_NETWORK must be one of main or test");
	}
	...
	private toAtomicKta(amount: string) {
		...
		if (!/^\d+(\.\d{1,9})?$/.test(normalized)) {
			throw new BadRequestException("Amount must be a positive KTA value with up to 9 decimals");
		}

		const [whole, fractional = ""] = normalized.split(".");
		const atomic = BigInt(whole) * 1_000_000_000n + BigInt(fractional.padEnd(9, "0"));
		if (atomic <= 0n) throw new BadRequestException("Amount must be greater than 0");
		return atomic;
	}

	private extractBlockHash(value: unknown): string | undefined {
		const data = value as any;
		const hash = data?.blocks?.[0]?.hash ?? data?.voteStaple?.blocks?.[0]?.hash ?? data?.blockHash ?? data?.hash;
